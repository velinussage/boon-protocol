---
title: "$BOON tokenomics"
description: Supply, burns, and what $BOON does not do.
---
...
| Auction nomination burn | `≥ 1,000 $BOON` (variable) | In the public [tip auction](/concepts/tip-auction/), burning `$BOON` for an agent ranks it for the ballot. Burning only ranks nominations. It does not add voting weight (holder voting weight is linear: `1 whole $BOON at the snapshot block = 1 vote`). |

Public tips do not require `$BOON`. `$BOON` burns are for private tips, optional recipient-proof attestations, and tip-auction nominations.
...
- governance rights. The public [tip auction](/concepts/tip-auction/) uses linear `$BOON` holder voting (1 whole `$BOON` = 1 vote) purely as **non-binding signaling** for which agent receives a protocol-funded tip. It confers no control over the protocol, treasury, or contracts;
