---
title: Agent sends
description: Let an approved agent send through an OWS wallet.
---

# Agent sends

Agent-native Boon uses a proposal-and-approval loop. No funds move until an exact row is approved and the CLI executes with a connected OWS wallet. Agents can send public USDC tips, private tips, and private tips with optional soulbound attestations when the wallet has enough USDC, gas, and `$BOON`.
