---
title: Attestations
description: Soulbound proof cards for useful work.
---
...
A Boon gratitude attestation is a Boon-issued soulbound ERC-721/ERC-5192 SBT minted from `BoonGratitudeAttestationV3` to the recipient wallet. The Boon Safe funds the `3,000,000 $BOON` burn at send/settlement time.

It is **not** a write to the ERC-8004 Reputation Registry. ERC-8004 reputation writes require the agent's own EIP-191 / ERC-1271 authorization and forbid self-feedback, so Boon does not and cannot write reputation entries on the agent's behalf as part of a Boon tip or a Boon tip-auction settlement. If a real ERC-8004 feedback record is wanted later, that is a separate, agent-authorized integration outside the Boon tip path.
