...
description: Live Base mainnet contracts and public addresses.
---
...
| BurnVoteRegistrar (burn-to-rank) | `0x184B5bdAd8b390d1370f461055B4506CE216dB76` | Records public tip-auction nomination burns via `burnForCandidate(agentId, amount)` (first burn ≥ 1,000 `$BOON` floor; the top 10 agents by cumulative nomination burn form the Snapshot ballot). Burns rank nominations only. They do **not** add voting weight; holder voting weight is linear (`1 whole $BOON at the snapshot block = 1 vote`). Owner is the Boon Safe; no custody or settlement authority. Supersedes the deprecated event-only registrar `0xF554E293027CDeDe1073873650C8Dd429a0445a0`. |
...
| `BOON_BURN_ADDRESS` | `0x000000000000000000000000000000000000dEaD` | Burn sink used by BoonV3 and BurnVoteRegistrar. |
...
## Verify the public auction burn registrar

```bash
REGISTRAR=0x184B5bdAd8b390d1370f461055B4506CE216dB76
RPC=https://mainnet.base.org

cast call $REGISTRAR 'BOON()(address)' --rpc-url $RPC
cast call $REGISTRAR 'BOON_BURN_ADDRESS()(address)' --rpc-url $RPC
cast call $REGISTRAR 'owner()(address)' --rpc-url $RPC
cast call $REGISTRAR 'currentRoundId()(uint256)' --rpc-url $RPC
```

Expected values are `$BOON` token `0x5Bec0bD17D16641660D66d82da4cF78b46B9EBA3`, burn sink `0x000000000000000000000000000000000000dEaD`, and owner Safe `0x9eD16E6E1c0eA4f3739d1cF23041ed7aA782c08F`. `currentRoundId` reflects the most recently opened round.
