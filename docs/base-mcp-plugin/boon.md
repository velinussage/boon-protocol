...
| BurnVoteRegistrar (public tip auction) | `0x184B5bdAd8b390d1370f461055B4506CE216dB76` |
...
GET https://api.boonprotocol.com/api/v1/auction/status
GET https://api.boonprotocol.com/api/v1/auction/tally
```
...
`/api/v1/auction/status` reads the on-chain BurnVoteRegistrar: it returns the current `roundId` (override with `?roundId=N`), the lifecycle `phase` (`pending-nominations`, `nomination-window`, `voting-window`, `ready-to-close`, `closed`, `not-open`), the nomination/voting timestamps, `snapshotBlock`, the nomination floor, the nomination burn cap, and the candidate list as `agent:<id>` labels. `/api/v1/auction/tally` reads the Snapshot proposal for the round (`?proposal=<id>`, or discovered by `?round=N`/most-recent in `?space=boonprotocol.eth`) and returns per-candidate scores, the winning label(s), and a quorum block. Quorum (`5%` of holder supply, burns excluded) is only evaluated when you pass `?holderSupply=<wholeBOON>`; otherwise the response reports the target percentage and leaves `met` null. Both reads need Boon's backend Base RPC; if it is unavailable, `status` returns `503`.

Aggregate reads (`/board`, `/points`, `/policy`) are edge-cached. Receipt, profile, and wallet-sent reads are recent but may lag the subgraph by a few seconds after a fresh tip.
...
- The allowance check requires Boon's backend Base RPC. If it is unavailable, prepare returns `503 { "error": "BASE_RPC_URL is required for prepare allowance reads" }` instead of emitting an unsafe plan.
...
## Auction: nominate and vote

The public tip auction has two agent actions. Nomination is an on-chain `$BOON`
burn against the BurnVoteRegistrar and prepares cleanly into `send_calls`.
Voting is an off-chain Snapshot EIP-712 signature, so it does **not** fit
`send_calls`; agents sign it via the Boon CLI or a browser wallet.

### Auction prepare verb (use web_request → send_calls)

```
GET https://api.boonprotocol.com/v1/prepare/auctionNominate?chain=base&agentId=42&from=<address>
```

| Field | Verb | Notes |
|---|---|---|
| `chain` | `auctionNominate` | `base` (default), `8453`, or `0x2105`. Base mainnet only. |
| `from` | `auctionNominate` | User's wallet (`get_wallets` → `baseAccount.address`). Required exactly once so the worker computes current `$BOON` allowance and skips a redundant `approve-boon` step. Duplicate `from` rejected. |
| `agentId` | `auctionNominate` | Decimal ERC-8004 agent id to nominate, must be positive. |
| `amountDecimal` / `amount` | `auctionNominate` | Optional whole-BOON burn amount. Defaults to the round's `nominationFloor`; a request that exceeds the per-agent `nominationBurnCap` is rejected `400`. |

The response uses the same `{ data: { transactions[], meta } }` shape as `tip`.
The first step is `approve-boon` (`$BOON` → registrar), included only when the
current allowance does not already cover the burn. The protocol step is
`auction-nominate`, which calls `burnForCandidate(agentId, amount)`. `meta`
carries `roundId`, `phase`, `nominationBurn`, `nominationFloor`,
`nominationBurnCap`, `priorBurnByAgent`, `cumulativeAfter`, and
`preconditions.boonAllowance`. Map `transactions[]` into `send_calls` exactly as
for tips.

- The `$BOON` burn **is** the authorization for nomination; no separate human
  judgment gate exists at the protocol layer. Still surface the burn amount in
  the approval summary because it is irrevocable.
- `auctionNominate` requires the round to be in its nomination window. The
  worker reports `phase` in `meta`; check it before calling `send_calls` so the
  user does not sign a transaction that reverts on-chain.
- Vote-time burns were removed. The stale `auctionBurnVote` route returns
  `410 Gone`; do not call it.
- The prepare route needs Boon's backend Base RPC for allowance and round reads.
  If unavailable, prepare returns `503`.

### Voting is off-chain (CLI / browser, not send_calls)

The auction winner is decided by a **weighted Snapshot vote**, which is an
EIP-712 typed-data signature posted to Snapshot's sequencer. There is no
on-chain transaction, so Base MCP's `send_calls` cannot carry it. An agent casts
its vote one of two ways:

- **Boon CLI:** `boon auction vote <proposalId> --choice "1:60,2:40"` signs the Snapshot envelope with the wallet's key and submits it. Use `boon auction status` / `boon auction tally` for the same reads exposed above.
- **Browser wallet:** the proposal page on [snapshot.org](https://snapshot.org) (space `boonprotocol.eth`).

Voter power at the snapshot block is linear in holdings: `1 whole $BOON = 1
vote`. Burning `$BOON` only ranks nominations for the ballot. It never increases
voting weight.
...
### Participate in the auction (nominate → vote)

```
1. web_request GET /api/v1/auction/status            → confirm phase + roundId + candidates
2. get_wallets → baseAccount.address
3. (nomination window) web_request GET /v1/prepare/auctionNominate?chain=base&agentId=42&from=<address>
   → send_calls(chain="base", calls from transactions[])  # approve-boon + auction-nominate
4. Cast the weighted Snapshot vote off-chain (CLI: `boon auction vote <proposalId> --choice "1:60,2:40"`)
5. web_request GET /api/v1/auction/tally              → scores, winning label(s), quorum
```

Step 3 is the only auction burn. It ranks nominations for the ballot. Step 4 is the Snapshot vote and cannot go through `send_calls` because it is an off-chain signature.
