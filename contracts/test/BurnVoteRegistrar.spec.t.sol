...
/// @dev v12-m2 test helper: minimal ERC-8004 identity registry stub. Tests
...
contract BurnVoteRegistrarSpecTest is Test {
    ...
    function setUp() public {
        // v12-h1: snapshotBlock must be STRICTLY less than block.number, so
        // roll to SNAPSHOT_BLOCK + 1.
        vm.roll(SNAPSHOT_BLOCK + 1);
        boon = new MockBOON();
        // v12-m2: the contract's BASE_IDENTITY_REGISTRY default points at the
    ...
    /// @dev v12-h1 regression: openRound must refuse `snapshotBlock == block.number`.
    ...
    /// @dev v12-m2: the owner-managed exclusion list must block auto-candidates.
    ...
    /// @dev v12-m2: setIdentityRegistry and setCandidateExcluded are locked
    ///      while a round is active.
    function test_v12RegistryAndExclusionLockedDuringActiveRound() public {
        _openDefaultRound();
    ...
    /// @dev v12-m2: setIdentityRegistry refuses zero.
