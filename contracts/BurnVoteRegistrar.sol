...
/// @dev v12-m2: minimal ERC-8004 identity-registry interface — only the
...
///      There is NO burn term in the vote. Vote-time burn amplification was
///      DROPPED (Tyler, 2026-05-29); the sqrt dampener was then dropped in favor
///      of linear holdings (Tyler, 2026-05-30). Burns serve exactly one job:
///      NOMINATION RANKING. The holder vote (linear holdings at the snapshot
///      block) confers legitimacy; the nomination burn only gates and ranks the ballot.
///
/// @dev Ballot selection is OFF-CHAIN and reproducible from the
///      `NominationBurnAdded` events:
///        score(agent) = nominationBurnByAgent[round][agent]  (RAW cumulative burn)
...
///      NOT enforce or apply `nominationBurnCap` on-chain. `nominationBurnCap`
///      is legacy round metadata; verifiers must ignore it for ranking.
contract BurnVoteRegistrar {
    ...
    /// @dev v12-m2: ERC-8004 identity registry. Every nominated/auto agent id
    ...
    /// @dev v12-m2: owner-curated exclusion list. Even if an agent id resolves
    ...
    ///      The off-chain ranker uses this raw total directly; on-chain we keep
    ///      the uncapped total so the full burn history is reproducible.
    ...
    /// @dev v12-m2. Locked while a round is active so a mid-round registry
    ...
    /// @dev v12-m2. Lets the operator publicly blacklist agent ids that are
    ...
    function openRound(
        ...
        // v12-h1 fix (was adv-005): snapshotBlock must reference a STRICTLY
        ...
        // nominationBurnCap is legacy metadata, but keep cap >= floor so old
        // round records and scripts never publish internally inconsistent
        // values;
        ...
        // v12-m2: refuse to open a round if the identity registry is unset.
    ...
    function _addCandidate(
        ...
        // v12-m2: checks the ERC-8004 identity registry resolves the id and
    ...
    /// @dev v12-m2: candidate-id gate. Refuses zero, excluded ids, and any id
