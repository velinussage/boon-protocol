#!/usr/bin/env node
/**
 * boon CLI — OWS-funded agent tipping
 *
 *   boon wallet connect ows --wallet <name>   — select an OWS agent wallet directly
 *   boon wallet current                       — show OWS address + Base USDC balance
 *   boon wallet disconnect                    — forget the selected OWS wallet
 *
 *   boon doctor                               — local readiness checks
 *   boon tip <handle> <amount> "<note>"       — send a tip via OWS; client-side guardrails first
 *   boon tip --dry-run ...                    — preview without moving funds
 *   boon history [handle]                     — local tip ledger
 *   boon weekly                               — entry point for agent-driven weekly review
 *
 * Signing model: Boon never holds a private key. It builds contract calldata
 * (`USDC.approve(Boon, amount)` if needed, then `Boon.tip(...)`) and sends it
 * through the official OWS Node/native binding. OWS verifies the ows_key_...
 * API token, enforces attached wallet policies, decrypts only inside its
 * signing path, signs/broadcasts, and wipes key material.
 *
 * Guardrails (~/.boon/config.json):
 *   { "maxUsdcPerDay": "50", "maxUsdcPerTip": "10",
 *     "minSecondsBetweenTips": 60, "dryRunInCi": true }
 *
 * USDC on Base: 0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913
 * Boon on Base: 0xfb6662AdaF0611a94322634d5B86203Cfb59d5e8
 *
 * Backend dependency: dry-runs, local guardrails, contract calldata, and live
 * OWS sends use Base RPC + OWS. The default hosted API is only used for
 * convenience reads such as wallet USDC balance and links back to the hosted
 * app; balance reads fall back to direct RPC when the API is unavailable.
 *
 */

import { Command } from "commander";
import { readFileSync, realpathSync } from "node:fs";
import { readFile, writeFile, mkdir, appendFile, open, rename, stat, unlink, chmod } from "node:fs/promises";
...
import {
  getOwsWallet,
  signAndSendOwsContractCall,
  createOwsWallet,
  ensureBoonBasePolicy,
  createOwsAgentKey,
  listOwsAgentKeys,
  revokeOwsAgentKey,
  BOON_BASE_POLICY_ID,
...
// $BOON token live on Base mainnet — burned when mintAttestation=true to mint the SBT.
const BOON_TOKEN_BASE_MAINNET: Address = "0x5Bec0bD17D16641660D66d82da4cF78b46B9EBA3";
// Mirrors private-tip.ts: BoonV3 ATTESTATION_BURN immutable. Burned when
// mintAttestation=true on any tip path, used to mint a soulbound
// proof-of-thanks NFT on the recipient. Public-tip paths only burn this
// amount; private-tip paths also burn PRIVATE_TIP_BURN on top.
...
interface CreateAgentKeyOptions {
  wallet?: string;
  name?: string;
  policy?: string;
  expires?: string;
...
function configuredBoonToken(settings: Settings): Address {
  return configuredAddress("BOON_TOKEN_ADDRESS", settings.boonToken, BOON_TOKEN_BASE_MAINNET, "$BOON token");
}
...
/**
 * Read the USDC balance for `wallet`. Prefers the hosted Boon API (so the
 * operator doesn't need RPC creds locally). Falls back to direct RPC read if
 * the API is unreachable or returns a non-2xx.
 */
...
/**
 * Read the wallet passphrase without going through sage or any system keychain.
 * Order: --passphrase-file, --passphrase-fd, then a non-echoed interactive
 * prompt. Automation should use the file/fd forms so no TTY is required.
 */
async function readWalletPassphrase(options: {
  passphraseFile?: string;
  passphraseFd?: string;
  label?: string;
}): Promise<string> {
  if (options.passphraseFile) {
    const value = (await readFile(options.passphraseFile, "utf8")).trim();
    if (!value) throw new Error(`--passphrase-file ${options.passphraseFile} was empty`);
    return value;
  }
  if (options.passphraseFd) {
    const fd = Number.parseInt(options.passphraseFd, 10);
    if (!Number.isInteger(fd) || fd < 0) throw new Error("--passphrase-fd must be a non-negative integer");
    const value = readFileSync(fd, "utf8").trim();
    if (!value) throw new Error("--passphrase-fd stream was empty");
    return value;
  }
  if (!input.isTTY) {
    throw new Error(
      "no passphrase source: pass --passphrase-file <path> or --passphrase-fd <n> for automation, or run interactively.",
    );
  }
  // Non-echoed interactive prompt: mute the output stream while the secret is typed.
  const muted = { muted: false };
  const originalWrite = output.write.bind(output);
  (output as unknown as { write: typeof output.write }).write = ((chunk: unknown, ...rest: unknown[]) => {
    if (muted.muted) return true;
    return (originalWrite as (...args: unknown[]) => boolean)(chunk, ...rest);
  }) as typeof output.write;
  const rl = createInterface({ input, output, terminal: true });
  try {
    const question = rl.question(`${options.label ?? "OWS wallet passphrase"}: `);
    muted.muted = true;
    const answer = await question;
    muted.muted = false;
    output.write("\n");
    const value = answer.trim();
    if (!value) throw new Error("empty passphrase");
    return value;
  } finally {
    muted.muted = false;
    (output as unknown as { write: typeof output.write }).write = originalWrite;
    rl.close();
  }
}

function maskedTokenPrefix(token: string): string {
  return `${token.slice(0, 12)}…`;
}

async function walletCreate(name: string, options: CreateWalletOptions = {}): Promise<void> {
  if (!name) throw new Error("usage: boon wallet create <name>");
  const passphrase = await readWalletPassphrase({
    passphraseFile: options.passphraseFile,
    passphraseFd: options.passphraseFd,
    label: `passphrase for new wallet ${name}`,
  });
  const wallet = await createOwsWallet(name, passphrase);
  console.log(`✓ created OWS wallet ${wallet.name} (${wallet.id})`);
  console.log(`Base address: ${wallet.address}`);
  console.log(`Fund it with Base USDC, then: boon wallet connect ows --wallet ${wallet.name}`);
}

async function walletCreatePolicy(): Promise<void> {
  const id = await ensureBoonBasePolicy();
  console.log(`✓ Boon Base-mainnet policy present: ${id}`);
  console.log("Scope: deny-by-default, allowed chain eip155:8453 (Base mainnet).");
}

async function walletCreateAgentKey(options: CreateAgentKeyOptions): Promise<void> {
  if (!options.wallet) {
    throw new Error("usage: boon wallet create-agent-key --wallet <name> [--policy boon-base-mainnet-send] [--out <file>]");
  }
  const policyId = options.policy ?? BOON_BASE_POLICY_ID;
  if (policyId === BOON_BASE_POLICY_ID) {
    await ensureBoonBasePolicy();
  }
  const wallet = await getOwsWallet(options.wallet);
  const keyName = options.name ?? `${wallet.name}-agent-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}`;
  const passphrase = await readWalletPassphrase({
    passphraseFile: options.passphraseFile,
    passphraseFd: options.passphraseFd,
    label: `passphrase for wallet ${wallet.name}`,
  });
  const minted = await createOwsAgentKey({
    name: keyName,
    walletNameOrId: wallet.name,
    policyIds: [policyId],
    passphrase,
    expiresAt: options.expires,
  });

  const outPath = options.out ?? join(homedir(), ".ows", `${minted.name}.token`);
  await mkdir(join(outPath, ".."), { recursive: true }).catch(() => undefined);
  await writeFile(outPath, minted.token, { mode: 0o600 });
  await chmod(outPath, 0o600).catch(() => undefined);

  console.log(`✓ minted OWS agent key ${minted.name} (${minted.id})`);
  console.log(`  wallet: ${wallet.name} (${wallet.address})`);
  console.log(`  policy: ${policyId}`);
  console.log(`  expires: ${options.expires ?? "never"}`);
  console.log(`  token prefix: ${maskedTokenPrefix(minted.token)} (length ${minted.token.length})`);
  console.log(`  token written to: ${outPath} (chmod 600)`);
  console.log(`Use it with: BOON_OWS_API_KEY_FILE=${outPath} boon auction vote ...`);
  if (options.reveal) {
    // Only when explicitly requested (CI secret capture). Printed alone on stdout.
    console.log(minted.token);
  } else {
    console.log("Re-run with --reveal to print the raw token (for CI secrets); otherwise it stays in the --out file only.");
  }
}

async function walletListAgentKeys(options: { json?: boolean } = {}): Promise<void> {
  const keys = await listOwsAgentKeys();
  if (options.json) {
    console.log(JSON.stringify(keys, null, 2));
    return;
  }
  if (keys.length === 0) {
    console.log("No OWS agent keys in the vault.");
    return;
  }
  for (const key of keys) {
    console.log(`${key.id ?? "(no id)"}  ${key.name ?? "(no name)"}`);
    console.log(`  wallets: ${key.walletIds.join(", ") || "(none)"}`);
    console.log(`  policies: ${key.policyIds.join(", ") || "(none)"}`);
    console.log(`  expires: ${key.expiresAt ?? "never"}`);
  }
}

async function walletRevokeAgentKey(id: string): Promise<void> {
  if (!id) throw new Error("usage: boon wallet revoke-agent-key <id>");
  await revokeOwsAgentKey(id);
  console.log(`✓ revoked OWS agent key ${id}`);
}
...
// Ensures the OWS signer has approved `spender` to spend at least `minAllowance`
// of $BOON. Required when mintAttestation=true on the public tip path —
// BoonV3._burnBoon pulls ATTESTATION_BURN from msg.sender via transferFrom,
// and reverts with PermitFailedAndAllowanceInsufficient if neither permit nor
// allowance is sufficient. Mirrors ensureBoonUsdcAllowance.
async function ensureBoonBoonAllowance(
  settings: Settings,
  pub: ReturnType<typeof makePublicClient>,
  walletName: string,
  agentSigner: Address,
  ...
  const boonToken = configuredBoonToken(settings);
  const currentAllowance = await pub.readContract({
    address: boonToken,
    abi: ERC20_ABI,
    functionName: "allowance",
  ...
  console.log("granting Boon $BOON allowance from OWS signer (for ATTESTATION_BURN)…");
  const allowanceData = encodeFunctionData({
    abi: ERC20_ABI,
    functionName: "approve",
    args: [spender, maxUint256],
  ...
  const approveTx = await executeOwsContractWrite(
    ...
    walletName,
    {
      label: "boon ows $BOON approval",
      to: boonToken,
      dataHex: allowanceData,
      reason: `Allow Boon contract ${spender} to burn $BOON from the OWS signer for proof-of-thanks attestation`,
    },
  );
  console.log(`  tx: ${approveTx}`);

  await waitForAllowance(pub, boonToken, agentSigner, spender, minAllowance, "$BOON");
  return approveTx;
}
...
async function tip(
  ...
  if (options.mintAttestation && !useV3) {
    console.error(
      "--mint-attestation requires the v3 contract path. Set BOON_ACTIVE_CONTRACT=v3 or configure settings.activeContract='v3'.",
    );
    process.exit(1);
  }
  ...
  if (dryRun) {
    const preview = {
      ...
      ...(options.mintAttestation
        ? {
            boonBurn: ATTESTATION_BURN.toString(),
            boonBurnDecimal: formatUnits(ATTESTATION_BURN, 18),
          }
        : {}),
      calls: useV3
        ? isAgentRecipient
          ? [
              ...
              ...(options.mintAttestation
                ? [`$BOON.approve(BoonV3, ${ATTESTATION_BURN.toString()}) if needed [exact burn: 3,000,000 $BOON = ATTESTATION_BURN]`]
                : []),
              `BoonV3.tipAgent(agentId, expectedWallet, amount, note, ${Boolean(options.mintAttestation)}, emptyPermit)`,
            ]
          : [
              "USDC.approve(BoonV3, max) if needed",
              ...(options.mintAttestation
                ? [`$BOON.approve(BoonV3, ${ATTESTATION_BURN.toString()}) if needed [exact burn: 3,000,000 $BOON = ATTESTATION_BURN]`]
                : []),
              `BoonV3.tip(handleHash, displayHandle, expectedWalletOrZero=0x0, amount, note, ${Boolean(options.mintAttestation)}, emptyPermit)`,
            ]
        : isAgentRecipient
          ? [
    ...
    if (json) {
      ...
      if (options.mintAttestation) {
        console.log(`mintAttestation: true (burns ${formatUnits(ATTESTATION_BURN, 18)} $BOON → soulbound proof-of-thanks NFT on recipient)`);
      }
      console.log(
  ...
  try {
    ...
    if (options.mintAttestation) {
      if (!useV3) {
        throw new Error("--mint-attestation requires the v3 contract path. Set BOON_ACTIVE_CONTRACT=v3 or configure settings.activeContract='v3'.");
      }
      await ensureBoonBoonAllowance(settings, pub, walletName, agentSigner, tipContract, ATTESTATION_BURN);
    }

    console.log(
      useV3
        ? options.mintAttestation
          ? `tipping ${amountUsdc} USDC to ${handle} via BoonV3 (+ 3M $BOON burn for proof-of-thanks SBT)…`
        ...
        dataHex: useV3
          ? isAgentRecipient
            ? encodeFunctionData({
                ...
                args: [agentResolution!.agentId, agentResolution!.expectedWallet, amountWei, note, Boolean(options.mintAttestation), EMPTY_PERMIT],
              })
            : encodeFunctionData({
                abi: BOON_V3_ABI,
                functionName: "tip",
                args: [handleHash, handle, ZERO_ADDRESS, amountWei, note, Boolean(options.mintAttestation), EMPTY_PERMIT],
              })
          : isAgentRecipient
            ? encodeFunctionData({
                abi: BOON_V2_ABI,
...
async function weekly(): Promise<void> {
  console.log("weekly: this command is intended to be invoked by your agent harness.");
  console.log("");
  console.log("The agent should:");
  console.log("  1. Read skill/boon/SKILL.md in this repo");
  console.log("  2. Pull collaboration signal from past 7 days (PRs, replies, etc.)");
  console.log("  3. Propose 1-3 boons in the format the skill specifies");
  console.log("  4. Run `boon tip --dry-run <handle> <amount> <note>` for preflight");
  console.log("  5. On operator approval, call `boon tip --yes --approval-id <id> <handle> <amount> <note>` per boon");
}
...
wallet
  .command("create <name>")
  .description("Create a new OWS agent wallet in the local vault (no sage)")
  .option("--passphrase-file <path>", "read the new wallet passphrase from a file (mode 0600)")
  .option("--passphrase-fd <n>", "read the new wallet passphrase from a file descriptor")
  .action((name: string, options: CreateWalletOptions) => {
    walletCreate(name, options).catch((err) => {
      console.error(err.message ?? err);
      process.exit(1);
    });
  });
...
wallet
  ...
  .description("Ensure the boon-base-mainnet-send OWS policy exists (deny-by-default, Base only)")
  .action(() => {
    walletCreatePolicy().catch((err) => {
      console.error(err.message ?? err);
      process.exit(1);
    });
  });
...
wallet
  .command("create-agent-key")
  .description("Mint a policy-scoped ows_key_... agent token bound to a wallet (no sage)")
  .requiredOption("--wallet <name>", "OWS wallet name or id to bind the key to")
  .option("--name <name>", "agent key display name")
  .option("--policy <id>", "policy id to scope the key to", BOON_BASE_POLICY_ID)
  .option("--expires <iso>", "ISO-8601 expiry timestamp; omit for no expiry")
  .option("--out <file>", "write the raw token to this file (chmod 600)")
  .option("--reveal", "also print the raw token to stdout (for CI secrets)")
  .option("--passphrase-file <path>", "read the wallet passphrase from a file (mode 0600)")
  .option("--passphrase-fd <n>", "read the wallet passphrase from a file descriptor")
  .action((options: CreateAgentKeyOptions) => {
    walletCreateAgentKey(options).catch((err) => {
      console.error(err.message ?? err);
      process.exit(1);
    });
  });
...
wallet
  .command("list-agent-keys")
  .description("List OWS agent keys in the vault (tokens are never shown)")
  .option("--json", "machine-readable output")
  .action((options: { json?: boolean }) => {
    walletListAgentKeys(options).catch((err) => {
      console.error(err.message ?? err);
      process.exit(1);
    });
  });
...
wallet
  .command("revoke-agent-key <id>")
  .description("Revoke (delete) an OWS agent key by id")
  .action((id: string) => {
    walletRevokeAgentKey(id).catch((err) => {
      console.error(err.message ?? err);
      process.exit(1);
    });
  });
...
program
  ...
  .option("--mint-attestation", "burn 3M $BOON to mint a soulbound proof-of-thanks NFT on the recipient (V3 only)")
...
claim
  ...
  .option("--json", "machine-readable output")
  .option("--forget", "delete the local in-flight claim session without contacting the Boon API")
...
program
  .command("weekly")
  .description("Weekly review entry point (invoked by agent)")
  .action(() => {
    weekly().catch((err) => {
      console.error(err.message ?? err);
      process.exit(1);
...
let invokedAsProgram = false;
try {
  invokedAsProgram = Boolean(process.argv[1]) &&
    realpathSync(process.argv[1]!) === realpathSync(fileURLToPath(import.meta.url));
} catch {
  invokedAsProgram = import.meta.url === pathToFileURL(process.argv[1] || "").href;
}
if (invokedAsProgram) {
  program.parse();
}
