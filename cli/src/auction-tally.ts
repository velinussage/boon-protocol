...
//
// CLI-local TypeScript port of the *pure* auction-settlement math that lives in
// the canonical read-only tally oracle. The
// `boon auction settle-prepare` command (U8) consumes this module so the CLI
// can reproduce, byte-for-byte, the Safe Transaction Builder batch the oracle
// emits — without re-deriving the tally math.
//
// Equivalence target with the oracle is *semantic*: same winner, same
// per-candidate scores, same calldata bytes. This file is NOT a source-level
// copy.
//
...
// (the sqrt was dropped — it taxed the accumulation the token rewards and, being
// Sybil-trivial, bought no real anti-whale protection). Burns serve ONE job —
// nomination ranking — handled on-chain by the registrar slice and consumed here
...
//   score(agent) = nominationBurnTotal(agent)   (RAW cumulative burn — no cap)
//   finalists    = TOP N by score desc; tiebreak firstBurnBlock asc, then
//                  agentId asc.
// Data source: the indexed auction-candidate fields `nominationBurnTotal`
...
// deterministic fixture and never depends on a live hosted index.
//
...
// NOMINATION_BURN_CAP_WEI: VESTIGIAL for ranking — nomination is now a pure
//   burn-to-rank auction (score = RAW cumulative burn). The cap is retained only
//   as published round metadata / the registrar's `cap >= floor` sentinel; it no
//   longer bounds the score.
...
/** Legacy burn input shape retained only for compatibility; ignored. */
export interface BurnVoteEvent {
  voter: string;
  /** Cumulative burned wei after this legacy event for this voter/round. */
  cumulativeAfter: string | bigint;
}

/** An on-chain candidate as exposed by the hosted auction projection. */
...
/**
 * Adapter the future `settle-prepare` command implements to feed the pure
 * tally. All blocking / non-deterministic I/O is funnelled through here so the
 * pure math stays offline-testable. Shaped to what U8 needs; intentionally
 * minimal.
 */
...
/**
 * Integer square root of a non-negative bigint (Newton's method). Retained as a
 * generic, unit-tested utility export; NOT part of the voter-power path anymore
 * (voting is linear — 1 $BOON = 1 vote — as of 2026-05-30).
 */
export function sqrtBigInt(value: bigint): bigint {
  if (value < 0n) throw new Error("sqrtBigInt only accepts non-negative values");
...
export interface BuildTallyInput {
  ...
  votes: SnapshotVote[];
  /**
   * Accepted but IGNORED. Voting weight is LINEAR in holdings (1 $BOON = 1 vote);
   * burns no longer contribute to voter power (they drive nomination ranking only
   * — see `selectFinalists`). Kept optional so existing callers still type-check.
   */
  burnVotes?: BurnVoteEvent[];
  ...
  /** Optional explicit U14-selected winner agentId when the top set ties. */
 ...
 * term), and selects a winner (unique top, U14-selected, or none). Pure given
 * `readBalanceAt`.
 */
export async function buildTally(input: BuildTallyInput): Promise<TallyResult> {
  ...
  if (winnerAgentId) {
    ...
    if (!topSet.some((row) => row.label === selected.label)) {
      throw new Error(`winnerAgentId ${winnerAgentId} is not in the U14 top set`);
    }
    winner = selected;
...
// ── Deterministic vote-tie resolution (nomination-burn tiebreak) ─────────────

/** A tied candidate's nomination-burn tiebreak data. */
export interface TieBreakInput {
  agentId: string;
  /** RAW cumulative nomination burn (wei) — no cap clamp. */
  burnWei: bigint;
  /** Block the agent first crossed the floor; null sorts LAST. */
  firstBurnBlock: bigint | null;
}

/**
 * Deterministic tiebreak for a vote-score TIE among candidate agents. Mirrors
 * `selectFinalists`' nomination ranking EXACTLY: highest RAW nomination burn
 * wins, then earliest `firstBurnBlock` (null sorts last), then lowest agentId.
 * Pure + reproducible offline — the same inputs always yield the same winner, so
 * holders can independently verify the autonomous tie resolution.
 *
 * The CALLER must only apply this to a PURE agent tie (no Abstain in the top
 * set): an Abstain that leads or ties the top agent is never auto-resolved.
 */
export function resolveVoteTieByNominationBurn(tied: TieBreakInput[]): TieBreakInput {
  if (tied.length === 0) throw new Error("resolveVoteTieByNominationBurn: empty tie set");
  const sorted = [...tied].sort((a, b) => {
    if (a.burnWei !== b.burnWei) return a.burnWei > b.burnWei ? -1 : 1; // burn desc
    if (a.firstBurnBlock === null && b.firstBurnBlock === null) {
      ...
      return a.firstBurnBlock < b.firstBurnBlock ? -1 : 1; // earliest first
    }
    const ai = BigInt(a.agentId);
    const bi = BigInt(b.agentId);
    return ai === bi ? 0 : ai < bi ? -1 : 1; // lowest agentId
  });
  return sorted[0]!;
}
...
/**
 * Per-candidate nomination-burn data derived from the registrar's
 * `NominationBurnAdded` events. All wei values are strings/bigints to avoid
 * float drift.
 */
export interface NominationCandidate {
  agentId: string | number;
  /**
   * Cumulative RAW BOON wei burned for this agent (`nominationBurnTotal`). An
   * auto-seeded candidate that was
   * never burned for has `0`.
   */
  nominationBurnTotal: string | bigint;
  /**
   * Block at which this agent first crossed the nomination floor
   * (`firstBurnBlock`). NULL/undefined for an auto-seeded
...
export interface FinalistRow {
  agentId: string;
  /** Raw cumulative burn (wei). */
  nominationBurnTotalWei: bigint;
  /** Ranking score (wei) = RAW cumulative burn (no cap clamp). */
  scoreWei: bigint;
...
export interface SelectFinalistsInput {
  ...
  /** Legacy cap metadata in wei (default 10,000 BOON). Ranking ignores it. */
 ...
 *                  agentId asc
 *
 * Reconciled with the registrar and indexed projection (2026-05-29): consumes
 * `nominationBurnTotal` (raw cumulative burn) and `firstBurnBlock`; the per-round floor is
 * `AuctionRound.nominationFloor`. The per-round `nominationBurnCap` is VESTIGIAL
 * for ranking (the contract still enforces `cap >= floor` at openRound, so we
 * assume cap >= floor) but no longer bounds the score.
 *
 * Auto-seeded candidates that were NEVER burned for arrive with
 * `nominationBurnTotal = 0` and `firstBurnBlock = null`. They score 0 and rank
 * LAST — they only reach the ballot if fewer than
 ...
 * function (it scores whatever candidate set it is handed). It has no live
 * hosted-index dependency, so feeding it candidate rows is fully reproducible.
 */
export function selectFinalists(input: SelectFinalistsInput): SelectFinalistsResult {
  ...
  for (const c of input.candidates) {
    ...
    // Pure burn-to-rank: score = RAW cumulative burn (no cap clamp). The cap
    // param is retained for signature compat (and the >= floor sentinel the
    // registrar enforces) but no longer bounds the score.
    const scoreWei = total;
  ...
  return {
    ...
    meta: {
      ...
      description:
        ...
        `(linear holder power, 1 $BOON = 1 vote; nomination burns only rank the ballot) via the canonical read-only tally oracle. ` +
        `The Safe (${BOON_SAFE}) MUST independently rerun ` +
        `the script and confirm the winner before signing.`,
...
export function buildTallyMarkdown(input: BuildTallyMarkdownInput): string {
  ...
  lines.push("");
  lines.push(`Generated: \`${ts}\` by the canonical read-only tally oracle.`);
  lines.push("");
  ...
  if (candidates.length === 0) {
    lines.push("_(No candidates indexed; the hosted index may be empty or not yet synced.)_");
  } else {
  ...
  if (!w) {
    ...
    lines.push(`- U14 top set: ${tally.topLabels.map((label) => `\`${label}\``).join(", ") || "—"}`);
  } else {
