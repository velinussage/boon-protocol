...
async function parseApiResponse<T>(resp: Response): Promise<T> {
  ...
  if (!resp.ok) {
    ...
    throw new ClaimExit(body?.error ?? code, mapApiExitCode(code, resp.status), code);
  }
  if (!body) throw new ClaimExit("Boon API returned an empty response", 75, "empty_response");
  return body as T;
}

function mapApiExitCode(code: string, status: number): number {
...
function apiErrorCopy(code: string, body?: CompleteResponse): string {
...
async function startDevice(apiUrl: string, handle: string, recipient: Address): Promise<StartResponse> {
  ...
  return await parseApiResponse<StartResponse>(resp);
}
...
async function pollDevice(apiUrl: string, deviceCode: string): Promise<PollResponse> {
  ...
  return await parseApiResponse<PollResponse>(resp);
}
...
async function completeClaim(
  ...
  if (!body) throw new ClaimExit("Boon API returned an empty claim response", 75, "empty_response");
  if (!resp.ok && resp.status !== 202) {
    const code = body.code ?? body.error ?? String(resp.status);
    throw new ClaimExit(apiErrorCopy(code, body), mapApiExitCode(code, resp.status), code);
  }
  ...
  try {
    ...
    if (status === 202 || code === "claim_already_in_progress") {
      jsonLine(options, {
        ...
        message: apiErrorCopy("claim_already_in_progress", body),
        exitCode: 0,
      });
      human(options, apiErrorCopy("claim_already_in_progress", body));
      process.exit(0);
    }
    if (code === "below_minimum_claim" || body.status === "noop") {
      jsonLine(options, {
        ...
        message: apiErrorCopy("below_minimum_claim", body),
        exitCode: 0,
      });
      human(options, apiErrorCopy("below_minimum_claim", body));
      process.exit(0);
    }
