...
// Snapshot's signed-message relay (`POST /api/msg`) is served by the HUB, not a
// separate `seq.` host — seq.snapshot.org has no /api/msg route (404). This
// matches the canonical proposal publisher, which posts ballots to
// `${hub}/api/msg` successfully. Override with BOON_SNAPSHOT_SEQ if Snapshot
// splits the relay onto a dedicated host later.
const DEFAULT_SNAPSHOT_SEQ = "https://hub.snapshot.org";
const TESTNET_SNAPSHOT_SEQ = "https://testnet.hub.snapshot.org";
...
const REGISTRAR_ABI = [
  ...
  { type: "function", name: "BOON", stateMutability: "view", inputs: [], outputs: [{ type: "address" }] },
  {
    type: "function",
    name: "getCandidates",
    stateMutability: "view",
...
// A non-zero, valid 20-byte address — anything else (empty, malformed, or the
// zero address) is treated as "unset". This is what stops a persisted
// settings.boonToken of 0x000…0 (or an env accidentally set to zero) from
// winning over a real token address.
function nonZeroAddress(raw: string | undefined): Address | null {
  if (!raw || !isAddress(raw)) return null;
  const addr = getAddress(raw);
  return addr === ZERO_ADDRESS ? null : addr;
}

// Resolve the $BOON token robustly. The zero address never wins, and an
// unavailable on-chain read never blocks the command.
//   precedence: NON-ZERO env BOON_TOKEN_ADDRESS
//             > NON-ZERO settings.boonToken
//             > registrar.BOON() on-chain read
//             > BOON_TOKEN_DEFAULT
// Never returns the zero address.
export async function resolveBoonToken(
  settings: Settings,
  pub: Pick<ReturnType<typeof makeClient>, "readContract">,
  registrar: Address,
): Promise<Address> {
  const fromEnv = nonZeroAddress(process.env.BOON_TOKEN_ADDRESS);
  if (fromEnv) return fromEnv;
  const fromSettings = nonZeroAddress(settings.boonToken);
  if (fromSettings) return fromSettings;
  try {
    const onChain = nonZeroAddress(
      (await pub.readContract({ address: registrar, abi: REGISTRAR_ABI, functionName: "BOON" })) as Address,
    );
    if (onChain) return onChain;
  } catch {
    // RPC unavailable / registrar without a BOON() view — fall through to the default.
  }
  return BOON_TOKEN_DEFAULT;
}
...
// The Snapshot sequencer accepts signed messages at /api/msg. snapshotSeq()
// returns the sequencer BASE (and honors the BOON_SNAPSHOT_SEQ override), so the
// vote POST must target /api/msg — appending it unless the override already does.
function snapshotMsgUrl(testnet: boolean): string {
  const base = snapshotSeq(testnet);
  return base.endsWith("/api/msg") ? base : `${base.replace(/\/+$/, "")}/api/msg`;
}
...
async function gqlProposal(hub: string, id: string): Promise<SnapshotProposal | null> {
  const query = `query Proposal($id: String!) {
    proposal(id: $id) {
      id title choices scores scores_total state type start end snapshot
    }
  }`;
...
// Fetch the most recent proposals in a space, with the fields the vote-discovery
// matcher needs (state / type / start / end / choices). Used by `auction vote`
// when no explicit proposalId is supplied. Distinct from gqlDiscoverProposal,
// which is tuned for the tally/status read; here we must surface active/pending
// proposals (not just closed ones) so an agent can vote in an open round.
async function gqlListProposals(hub: string, space: string): Promise<SnapshotProposal[]> {
  const query = `query Proposals($space: String!) {
...
// ---- vote proposal discovery (pure, exported for tests) ---------------------

export interface VoteDiscoveryWindow {
  votingOpensAt: bigint;
  votingClosesAt: bigint;
}

export interface VoteDiscoveryResult {
  proposal: SnapshotProposal;
  // How the proposal was selected, for transparent logging.
  reason:
    | "active-window-match"
    | "active-title-match"
    | "window-match"
...
// Round-title needle — byte-for-byte the convention used by settle-round.mjs
// (discoverProposalByRound) and the web app (findRoundProposal). Matches
// "Round 3", "Round #3", "Boon Round 3", case-insensitively.
function roundTitleNeedle(round: number | bigint): RegExp {
  return new RegExp(`(?:^|\\b)(?:boon\\s+)?round\\s*#?\\s*${round}\\b`, "i");
}

// True when the proposal's voting window (start/end, unix seconds) lines up with
// the on-chain round window. Snapshot proposal timing and the registrar's round
// timing are configured to agree, so a window match disambiguates when more than
// one proposal carries the round in its title (e.g. a re-published proposal).
// Allow a generous tolerance because the publisher rounds timestamps to minutes.
function windowMatches(proposal: SnapshotProposal, window: VoteDiscoveryWindow): boolean {
  if (proposal.start === undefined || proposal.end === undefined) return false;
  const toleranceSec = 6 * 60 * 60; // 6h — accommodate publish-time rounding/clock skew
  const startDelta = Math.abs(proposal.start - Number(window.votingOpensAt));
  const endDelta = Math.abs(proposal.end - Number(window.votingClosesAt));
  return startDelta <= toleranceSec && endDelta <= toleranceSec;
}

function isVotable(proposal: SnapshotProposal): boolean {
  // "active" = voting open; "pending" = scheduled but not yet open. Both are
  // candidates for a vote command (pending is reported as "not open yet" by the
  // caller). "closed" proposals are not votable.
  return proposal.state === "active" || proposal.state === "pending";
}

// Pure proposal selection for `auction vote`. Mirrors the web app's
// findRoundProposal ordering (prefer an active/votable proposal whose window
// matches the on-chain round) but is window-aware so it agrees with the
// registrar timing. Returns null when nothing matches the round.
//
// Exported so the matcher can be unit-tested with mocked proposal lists, with no
// network, wallet, or process exit.
export function discoverVoteProposal(
  proposals: SnapshotProposal[],
  round: number | bigint,
  window?: VoteDiscoveryWindow,
): VoteDiscoveryResult | null {
  const needle = roundTitleNeedle(round);
  const roundMatches = proposals.filter((p) => typeof p.title === "string" && needle.test(p.title));
  if (roundMatches.length === 0) return null;

  if (window) {
    const activeWindow = roundMatches.find((p) => isVotable(p) && windowMatches(p, window));
    if (activeWindow) return { proposal: activeWindow, reason: "active-window-match" };
    const anyWindow = roundMatches.find((p) => windowMatches(p, window));
    if (anyWindow) return { proposal: anyWindow, reason: "window-match" };
  }
  const active = roundMatches.find((p) => isVotable(p));
  if (active) return { proposal: active, reason: "active-title-match" };
  return { proposal: roundMatches[0]!, reason: "title-match" };
}
...
// ---- local keystore signing (opt-in, Foundry `cast`) ------------------------
// An explicit alternative to OWS signing for `nominate` and `vote`, engaged only
// by --sign-keystore --account <name>. Foundry `cast` decrypts the keystore and
// prompts for the password on
// the terminal, so the raw private key never enters this process. The OWS path
// stays the default — agent-mode policy enforcement is unchanged unless the flag
// is passed.
function keystoreAddress(account: string): Address {
  // Prefer the address field in the keystore JSON (no password) so dry-runs and
  // the pre-sign `from` resolution don't prompt — only the actual sign/send does.
  try {
    const raw = readFileSync(join(homedir(), ".foundry", "keystores", account), "utf8");
    const parsed = JSON.parse(raw) as { address?: string };
    if (parsed.address) return getAddress(parsed.address.startsWith("0x") ? parsed.address : `0x${parsed.address}`);
  } catch {
    /* fall through to cast (which decrypts → prompts for the password) */
  }
  try {
    const out = execFileSync("cast", ["wallet", "address", "--account", account], { encoding: "utf8" }).trim();
    return getAddress(out);
  } catch (e) {
    throw new Error(
      `could not resolve keystore "${account}" — import it with 'cast wallet import ${account} --interactive'. (${e instanceof Error ? e.message : String(e)})`,
    );
  }
}

// Sign a Snapshot vote EIP-712 payload via `cast wallet sign`. cast requires
// EIP712Domain in `types` + an explicit primaryType; the POSTed payload's `types`
// must NOT include EIP712Domain (Snapshot rejects it), so inject it only here.
function signVoteKeystore(account: string, message: Record<string, unknown>): Hex {
  const castInput = JSON.stringify({
    domain: SNAPSHOT_VOTE_DOMAIN,
    types: { ...SNAPSHOT_VOTE_TYPES, EIP712Domain: [{ name: "name", type: "string" }, { name: "version", type: "string" }] },
    primaryType: "Vote",
    message,
  });
  const tmpFile = join(mkdtempSync(join(tmpdir(), "boon-vote-sign-")), "typed-data.json");
  writeFileSync(tmpFile, castInput);
  try {
    return execFileSync("cast", ["wallet", "sign", "--account", account, "--data", "--from-file", tmpFile], {
      encoding: "utf8",
      stdio: ["inherit", "pipe", "inherit"],
    }).trim() as Hex;
  } catch (e) {
    throw new Error(`cast wallet sign failed: ${e instanceof Error ? e.message : String(e)}`);
  }
}

// Send a contract call signed by a Foundry keystore via `cast send` (waits for the
// receipt). Used for the local-signing nominate path. NOTE: an EIP-7702-delegated
// signer (e.g. 0xF121) can make cast misreport "insufficient funds for gas" at
// estimation — if that bites, pass --gas-limit or use the OWS path / app.
function castSendAndWait(input: { account: string; to: Address; sig: string; args: (string | bigint)[]; rpcUrl: string; gasLimit?: string; label: string }): Hex {
  const argv = ["send", input.to, input.sig, ...input.args.map(String), "--account", input.account, "--rpc-url", input.rpcUrl, "--json"];
  if (input.gasLimit) argv.push("--gas-limit", input.gasLimit);
  let out: string;
  try {
    out = execFileSync("cast", argv, { encoding: "utf8", stdio: ["inherit", "pipe", "inherit"] });
  } catch (e) {
    throw new Error(`${input.label} failed (cast send): ${e instanceof Error ? e.message : String(e)}`);
  }
  let receipt: { transactionHash?: string; status?: string };
  try {
    receipt = JSON.parse(out) as { transactionHash?: string; status?: string };
  } catch {
    throw new Error(`${input.label}: could not parse cast send output: ${out}`);
  }
  const txHash = (receipt.transactionHash ?? "0x") as Hex;
  const status = String(receipt.status ?? "").toLowerCase();
  if (status === "0x0" || status === "0" || status === "reverted" || status === "failed") {
    throw new Error(`${input.label} reverted: ${txHash}`);
  }
  return txHash;
}
...
async function auctionTally(options: TallyOptions): Promise<void> {
  ...
  if (options.holderSupply !== undefined) {
    ...
    // from quorum, so canonical quorum comes from the hosted auction tally read.
    // Report target + participation only when that evidence is not supplied.
  ...
  if (quorum.required === null) {
    console.log(`  quorum target: ${quorum.targetPercent}% of holder supply (burns excluded) — pass --holder-supply <whole BOON> to evaluate, or use the hosted tally read`);
  } else {
...
async function auctionNominate(agentIdArg: string, options: NominateOptions): Promise<void> {
  ...
  const pub = makeClient(settings);
  const registrar = registrarAddress(settings);
  const boonToken = await resolveBoonToken(settings, pub, registrar);
  ...
  // Ranking score is the RAW cumulative burn (the cap was removed — burns are
  // pure burn-to-rank). No clamp; later burns add to the agent's total.
  const countedAfter = priorBurn + burn;
  ...
  // NOTE: no nominationBurnCap reject — the cap was removed (pure burn-to-rank);
  // the contract does not enforce it and later burns add to the agent's total.
  ...
  // Resolve the signer: OWS wallet (default) or a local Foundry keystore (opt-in).
  const useKeystore = Boolean(options.signKeystore);
  let signer: Address;
  let walletName = "";
  if (useKeystore) {
    if (!options.account) throw new Error("--sign-keystore requires --account <name> (a `cast wallet import` keystore)");
    signer = options.from ? getAddress(options.from) : keystoreAddress(options.account);
  } else {
    const r = resolveOwsSigner(settings);
  ...
  if (balance < burn) throw new Error(`signer ${signer} $BOON balance too low: have ${formatUnits(balance, 18)}, need ${formatUnits(burn, 18)}`);

  if (useKeystore) {
    const rpcUrl = process.env.BASE_RPC_URL || settings.rpcUrl || DEFAULT_RPC_URL;
    const account = options.account as string;
    const current = (await pub.readContract({ address: boonToken, abi: ERC20_ABI, functionName: "allowance", args: [signer, registrar] })) as bigint;
    if (current < burn) {
      const approveAmt = parseAllowanceMode(options.allowanceMode) === "max" ? maxUint256 : burn;
      console.log(`granting $BOON allowance (${parseAllowanceMode(options.allowanceMode)})…`);
      castSendAndWait({ account, to: boonToken, sig: "approve(address,uint256)", args: [registrar, approveAmt], rpcUrl, gasLimit: options.gasLimit, label: "$BOON approval" });
    }
    console.log(`burning ${formatUnits(burn, 18)} BOON for agent:${agentId.toString()} from ${signer}…`);
    const txHash = castSendAndWait({ account, to: registrar, sig: "burnForCandidate(uint256,uint256)", args: [agentId, burn], rpcUrl, gasLimit: options.gasLimit, label: "BurnVoteRegistrar.burnForCandidate" });
...
// Network-bound resolver for `auction vote` when no proposalId is supplied.
// Determines the round (--round or on-chain currentRoundId), reads the round's
// voting window from the registrar, lists the space's proposals from the
// Snapshot hub, and selects the matching proposal via the pure discoverVoteProposal
// matcher. Emits agent-friendly errors when nothing matches.
async function resolveVoteProposal(input: {
  hub: string;
  space: string;
  settings: Settings;
  round?: string;
}): Promise<{ proposal: SnapshotProposal; roundId: bigint; reason: VoteDiscoveryResult["reason"] }> {
  const { hub, space, settings } = input;
  const pub = makeClient(settings);
  const registrar = registrarAddress(settings);
  ...
  const roundId =
    input.round !== undefined
      ...
      : ((await pub.readContract({ address: registrar, abi: REGISTRAR_ABI, functionName: "currentRoundId" })) as bigint);
  if (roundId <= 0n) {
    throw new Error("no auction round is open (currentRoundId is 0). Pass --round <id> or --proposal <id>.");
  }

  // On-chain round window disambiguates when more than one proposal carries the
  // round in its title. A non-existent round just yields a zero window, which the
  // matcher safely ignores (falls back to title matching).
  let window: VoteDiscoveryWindow | undefined;
  try {
    const round = await readRound(pub, registrar, roundId);
    if (round.exists) {
      window = { votingOpensAt: round.votingOpensAt, votingClosesAt: round.votingClosesAt };
    }
  } catch {
    // window is optional; fall through to title-only matching
  }

  const proposals = await gqlListProposals(hub, space);
  const result = discoverVoteProposal(proposals, roundId, window);
  if (!result) {
    throw new Error(
      `no Snapshot proposal found for round ${roundId.toString()} in space ${space}. ` +
        `The round may not have a published proposal yet — run \`boon auction status --round ${roundId.toString()} --snapshot\` ` +
        `to check, or pass --proposal <id> once it is published.`,
    );
  }
  return { proposal: result.proposal, roundId, reason: result.reason };
}

async function auctionVote(proposalIdArg: string | undefined, options: VoteOptions): Promise<void> {
  const dryRun = Boolean(options.dryRun) || process.env.BOON_DRY_RUN === "1";
  ...
  const hub = snapshotHub();
  const space = snapshotSpace(settings, options.space);
  // Signer: OWS wallet (default) or a local Foundry keystore (opt-in via --sign-keystore).
  const useKeystore = Boolean(options.signKeystore);
  let walletName = "";
  let signer: Address;
  if (useKeystore) {
  ...
  // Resolve the proposalId. Either it was passed explicitly (back-compat), or we
  // auto-discover the round's Snapshot proposal so an agent can vote natively
  // without a human pasting an id.
  let proposalId: string;
  let proposal: SnapshotProposal | null;
  let discovery: { roundId: bigint; reason: VoteDiscoveryResult["reason"]; state: string } | null = null;

  if (proposalIdArg) {
    proposalId = proposalIdArg;
    // Validate choice indices against the proposal's actual choices when reachable.
    proposal = await gqlProposal(hub, proposalId).catch(() => null);
  } else {
    const resolved = await resolveVoteProposal({ hub, space, settings, round: options.round });
    proposalId = resolved.proposal.id;
    proposal = resolved.proposal;
    discovery = { roundId: resolved.roundId, reason: resolved.reason, state: resolved.proposal.state };
    const how =
      resolved.reason === "active-window-match" || resolved.reason === "window-match"
        ? "title + on-chain voting window"
        : "round title";
    // Transparency line goes to stderr when --json so machine-readable stdout
    // stays clean (the resolved id + how are also in the JSON `discovered` block).
    const note = (line: string) => (options.json ? console.error(line) : console.log(line));
    note(
      `discovered proposal ${proposalId} for round ${resolved.roundId.toString()} (matched by ${how}; state=${resolved.proposal.state})`,
    );
    if (resolved.proposal.state === "pending") {
      note("  note: this proposal's voting window is not open yet (state=pending)");
    }
  }
  ...
  if (dryRun) {
    const preview = {
      ...
      discovered: discovery
        ? { roundId: discovery.roundId.toString(), matchedBy: discovery.reason, state: discovery.state }
        : null,
      from: signer,
      choice,
      ...
      reason,
      app,
      sequencer: snapshotMsgUrl(Boolean(options.testnet)),
  ...
  const sig = useKeystore
    ? signVoteKeystore(options.account as string, message)
    : await signTypedDataOws({
        wallet: walletName,
        // The OWS binding (alloy) — like cast — requires EIP712Domain in `types`
        // to resolve the domain separator, else it throws "unknown type:
        // EIP712Domain". Inject it for SIGNING only; the POSTed payload (below)
        // keeps the clean SNAPSHOT_VOTE_TYPES because Snapshot/ethers reject an
        // EIP712Domain entry in `types`.
        typedData: {
          domain: SNAPSHOT_VOTE_DOMAIN,
          types: {
            ...SNAPSHOT_VOTE_TYPES,
  ...
  const res = await fetch(snapshotMsgUrl(Boolean(options.testnet)), {
    method: "POST",
    headers: { "content-type": "application/json", accept: "application/json" },
...
export function registerAuctionCommand(program: Command): void {
  ...
  auction
    ...
    .option("--sign-keystore", "sign + send with a local Foundry keystore instead of the OWS wallet")
    .option("--account <name>", "Foundry keystore account name (with --sign-keystore), e.g. boon-deployer")
    .option("--from <address>", "keystore signer address (skips a decrypt prompt; must match the keystore)")
    .option("--gas-limit <n>", "explicit gas limit for the keystore send (e.g. for EIP-7702 signers)")
  ...
  auction
    ...
    .description("Cast a weighted Snapshot vote off-chain via EIP-712 (no browser wallet required); auto-discovers the round's proposalId when omitted")
    .option("--round <id>", "discover the proposal for this round (default: on-chain currentRoundId)")
    ...
    .option("--sign-keystore", "sign with a local Foundry keystore instead of the OWS wallet")
    .option("--account <name>", "Foundry keystore account name (with --sign-keystore), e.g. boon-deployer")
    .option("--from <address>", "keystore signer address (skips a decrypt prompt; must match the keystore)")
    .option("--json", "machine-readable dry-run output")
    .action((proposalId: string | undefined, options: VoteOptions) => {
