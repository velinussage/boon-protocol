...
const mintPath = join(root, "ows-mint.json");
const mockOws = join(root, "mock-ows.mjs");
// The raw token the mock createApiKey hands back, mirroring a real ows_key_... mint.
const mintedToken = `ows_key_${"b".repeat(64)}`;
mkdirSync(home, { recursive: true });
writeFileSync(
  mockOws,
  `import { appendFileSync, existsSync, readFileSync, writeFileSync } from "node:fs";
const callsPath = ${JSON.stringify(callsPath)};
const mintPath = ${JSON.stringify(mintPath)};
const txHashes = ${JSON.stringify(txHashes)};
const mintedToken = ${JSON.stringify(mintedToken)};
...
export function createWallet(name, passphrase) {
  writeFileSync(mintPath, JSON.stringify({ op: "createWallet", name, passphrase }));
  return getWallet(name);
}
let policyCreated = false;
export function getPolicy(id) {
  if (!policyCreated) throw new Error("policy not found");
  return { id, name: "Boon Base mainnet sends" };
}
export function createPolicy(policyJson) {
  policyCreated = true;
  appendFileSync(callsPath, JSON.stringify({ op: "createPolicy", policyJson }) + "\\n");
}
export function createApiKey(name, walletIds, policyIds, passphrase, expiresAt) {
  // Persist what we were asked to mint so the test can assert scoping. We do NOT
  // echo the token anywhere except the returned object.
  writeFileSync(mintPath, JSON.stringify({ op: "createApiKey", name, walletIds, policyIds, expiresAt, passphraseLen: String(passphrase).length }));
  return { token: mintedToken, id: "key-minted", name };
}
export function revokeApiKey(id) {
  appendFileSync(callsPath, JSON.stringify({ op: "revokeApiKey", id }) + "\\n");
}
...
try {
  ...
  // ── agent-key mint: boon mints a policy-scoped ows_key_... itself (no sage) ──
  const tokenOut = join(root, "minted.token");
  const mintEnv = {
    ...process.env,
    HOME: home,
    BOON_OWS_BINDING_PATH: mockOws,
    BOON_OWS_VAULT: join(home, ".ows"),
  };
  const passFile = join(root, "pass.txt");
  ...
  // The minted record was scoped to the boon policy + the resolved wallet id.
  const mint = JSON.parse(readFileSync(mintPath, "utf8"));
  assert.equal(mint.op, "createApiKey", JSON.stringify(mint));
  assert.deepEqual(mint.policyIds, ["boon-base-mainnet-send"], JSON.stringify(mint));
  assert.deepEqual(mint.walletIds, ["wallet-smoke"], JSON.stringify(mint));
  assert(mint.passphraseLen > 0, "passphrase should have reached createApiKey");

  // The raw token landed in the --out file at chmod 600.
  const tokenContents = readFileSync(tokenOut, "utf8");
  assert.equal(tokenContents, mintedToken, "token file should contain exactly the raw token");
  const mode = statSync(tokenOut).mode & 0o777;
  assert.equal(mode, 0o600, `token file mode should be 0600, got ${mode.toString(8)}`);

  // The token must NEVER be printed to stdout/stderr without --reveal.
  assert(!res.stdout.includes(mintedToken), "raw token must not appear on stdout");
  assert(!res.stderr.includes(mintedToken), "raw token must not appear on stderr");
  assert(res.stdout.includes(`${mintedToken.slice(0, 12)}…`), "masked prefix should be shown");
  assert(res.stdout.includes("boon-base-mainnet-send"), res.stdout);

  // create-policy is idempotent and reports the boon policy id.
  res = await runAsync(["wallet", "create-policy"], mintEnv);
  assert.equal(res.status, 0, res.stdout + res.stderr);
  assert(res.stdout.includes("boon-base-mainnet-send"), res.stdout);

  // list-agent-keys never prints a token.
  res = await runAsync(["wallet", "list-agent-keys", "--json"], mintEnv);
  assert.equal(res.status, 0, res.stdout + res.stderr);
  assert(!res.stdout.includes(mintedToken), "list must not print tokens");

  // --reveal opt-in prints the raw token (CI secret capture path).
  res = await runAsync(
    ["wallet", "create-agent-key", "--wallet", "smoke-ows", "--out", join(root, "r.token"), "--passphrase-file", passFile, "--reveal"],
    mintEnv,
  );
  assert.equal(res.status, 0, res.stdout + res.stderr);
  assert(res.stdout.includes(mintedToken), "--reveal should print the raw token");
} finally {
  await rpc?.close();
