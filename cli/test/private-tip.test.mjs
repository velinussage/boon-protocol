...
try {
  ...
  // JSON dry-run exposes the route-local call sequence for agent harnesses.
  {
    const res = run([
      "tip-private",
      "github:alice",
  ...
  // Dry-run reflects a manually configured local wallet mode without inventing key custody.
  {
    ...
    assert(
      body.calls.some((call) => call.includes("boonprotocol.com/send") || call.includes("BLOCKERS.md B1")),
      body.calls.join("; "),
    );
  }
