...
// Full happy path with a mocked Boon API: start -> poll approved -> claim complete.
await withServer(
  (req, res, rawBody) => {
...
async function expectCompleteError({ apiBody, apiStatus, expectedExit, expectedCode }) {
  await withServer(
    (req, res) => {
      const url = new URL(req.url ?? "/", "http://127.0.0.1");
      if (req.method === "POST" && url.pathname === "/auth/cli/device/start") {
      ...
      if (req.method === "POST" && url.pathname === "/claim/complete") {
        return json(res, apiStatus, apiBody);
      }
      return json(res, 404, { error: "not found" });
    },
    async (apiUrl) => {
...
await expectCompleteError({
  ...
  expectedExit: 75,
  apiStatus: 501,
  apiBody: { code: "relayer_not_enabled", error: "claim relayer not enabled" },
});
