...
export function SendWidget() {
      ...
      .then(([legacyProfileResult, v2LinkedWalletResult]) => {
        ...
        if (v2LinkedWalletResult.status === "rejected") {
          setRecipientClaimCheck({
            state: "error",
            handle: checkHandle,
            message: `Could not verify ${checkHandle}'s BoonV2 link status. Try again before signing.`,
          });
          return;
        }
        ...
        if (legacyLinkedWallet && isAddress(legacyLinkedWallet)) {
          setRecipientClaimCheck({
            state: "unlinked",
            handle: checkHandle,
            message: `${checkHandle} is linked on legacy Boon v1 but not BoonV2; recipient must link for BoonV2 before private tips or recipient proofs.`,
          });
          return;
        }
        ...
        setRecipientClaimCheck({
          state: "unlinked",
          handle: checkHandle,
          message: `${checkHandle} must claim/link a wallet on BoonV2 before private tips or recipient proofs.`,
        });
      })
      .catch(() => {
        if (cancelled) return;
  ...
  const privateTipIntentNotice = canQueuePrivateTipIntent && canonical
    ? `${canonical.handle} is not linked on BoonV2 yet. No funds move now; you’ll sign a private intent and return to the execution link once the recipient joins.`
    : null;
  ...
  function validateDraft(): string | null {
    ...
    if (!contract || contract === ZERO_ADDRESS) {
      if (usesV3) return "VITE_BOON_V3_CONTRACT is not configured yet.";
      if (usesV2) return "VITE_BOON_V2_CONTRACT is not configured yet.";
      return "VITE_BOON_CONTRACT is not configured yet.";
    }
    if (requiredBoonBurn > 0n) {
      ...
      if (!boonToken || boonToken === ZERO_ADDRESS) {
        return "VITE_BOON_TOKEN_ADDRESS is not configured yet.";
      }
    }
    return null;
  }
  ...
  const recipientActionBlockerLabel = (() => {
    ...
    return "Recipient needs BoonV2 link";
  })();
    ...
    if (!selected) {
      throw new Error(
        kind === "coinbase"
          ...
          : kind === "walletConnect"
            ? "Other wallet support is not configured. Set VITE_WC_PROJECT_ID, rebuild, and redeploy."
  ...
  async function refreshBalance(
    ...
    try {
      ...
      if (!resp.ok) throw new Error(`worker balance read returned ${resp.status}`);
  ...
  function startOnramp() {
    ...
    void (async () => {
      try {
        ...
        if (!resp.ok) throw new Error(`worker /onramp/session returned ${resp.status}`);
  ...
  async function resolveExpectedWallet(canonicalHandle: NonNullable<typeof canonical>): Promise<Address> {
    ...
    if (!linkedWallet || !isAddress(linkedWallet)) {
      throw new Error("Private social tips require a recipient wallet linked on BoonV2.");
    }
    throw new Error(
      `${canonicalHandle.handle} is linked on legacy Boon v1 but not BoonV2; recipient must link for BoonV2 before private tips or recipient proofs.`,
    );
  }
        ...
        <div>
          ...
          {provider === "agent" && (
            <div className="mt-3 rounded-md border border-faint bg-paper-deep/60 p-3 text-xs text-muted leading-relaxed">
              <p>
                Agent recipients use an <strong className="text-ink">ERC-8004 ID</strong>. It's the onchain
            ...
            <div>
              ...
              <p className="text-xs text-muted leading-relaxed mt-1">
                {usesV3
                  ...
                  : "Optional soulbound NFT for the recipient. GitHub/X handles must link on BoonV2 before a proof can mint; if they have not linked yet, you can sign a private intent now and return to the execution link once they join. Costs 3,000,000 $BOON."}
              </p>
            </div>
            <button
              type="button"
      ...
      {status === "queued" && queuedPrivateTipIntent && (
        <div className="mt-5 rounded-md border border-olive bg-olive-soft/50 p-4 text-sm leading-relaxed">
          <p className="text-success">
            ✓ Will execute when <span className="chip">{queuedPrivateTipIntent.handle}</span> joins BoonV2.
          </p>
