 ...
 *   4. The Worker/relayer links the handle and sweeps pending escrow.
 *
 * The claim completion endpoint is app-facing: if relay operations are not
 * configured, surface a recipient-safe explanation instead of raw Worker
 * status text.
 */
...
export function ClaimWidget() {
  ...
  // app can be deployed ahead of Worker rollout without stranding testers.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function loadClaimable() {
      ...
      try {
        ...
        if (!resp.ok) {
          throw new Error(`worker /api/claimable returned ${resp.status}`);
        }
  ...
  // the OAuth callback (worker-canonical); otherwise compute keccak256 of the
  // canonicalized handle locally so the banner still renders pre-callback hops.
  ...
  async function startOAuth(provider: OAuthProvider) {
    ...
    try {
      ...
      if (!resp.ok) {
        if (resp.status === 400) {
          throw new Error(
            "The deployed Worker still expects a wallet before OAuth. The identity-first claim API is not live yet.",
          );
        }
        throw new Error(`worker /auth/${provider}/start returned ${resp.status}`);
      }
      const data = (await resp.json()) as { authorizeUrl?: string };
      if (!data.authorizeUrl) throw new Error("worker did not return authorizeUrl");
    ...
    <div className="card p-6 md:p-8">
      ...
      <ol className="relative space-y-7">
        ...
        <TimelineStep
          ...
          help="Start with GitHub or X. Boon only asks the provider for your username, then the Worker returns a short-lived claim proof."
        >
          {step === 1 && (
...
function CashoutGuidance({
  ...
  // Pull the live USDC balance from the worker — the authoritative "did this
...
function claimCompleteErrorMessage(status: number, body: ClaimErrorResponse | null): string {
  ...
  return `Claim failed with Worker status ${status}. No funds moved.`;
}
