...
`boonprotocol.com` — landing, send, claim, board, profile, receipt, and attestation pages. Vite + React SPA. This repository builds the static app; live hosting is managed by the maintainers.
...
Live deployment configuration is maintained outside this repository. Contributors should validate static builds here and rely on the hosted `https://boonprotocol.com` / `https://api.boonprotocol.com` endpoints for production behavior.
